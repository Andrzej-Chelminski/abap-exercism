CLASS ltcl_itab_basics DEFINITION FOR TESTING RISK LEVEL HARMLESS DURATION SHORT FINAL.
  PRIVATE SECTION.
    DATA cut TYPE REF TO zcl_exercism_itab_basics.
    METHODS setup.
...
CLASS ltcl_itab_basics IMPLEMENTATION.
  ...
  METHOD setup.
    cut = NEW zcl_exercism_itab_basics( ).
  ENDMETHOD.
  ...
  METHOD test_fill_itab.
    ...
    DATA(expected_values) = VALUE zcl_exercism_itab_basics=>itab_data_type(
  ...
  METHOD test_sort_itab_empty.
    cl_abap_unit_assert=>assert_equals(
      ...
      exp = VALUE zcl_exercism_itab_basics=>itab_data_type( ) ).
  ENDMETHOD.
