CLASS ltcl_exercism_itab_nesting DEFINITION FOR TESTING RISK LEVEL HARMLESS DURATION SHORT FINAL.
  PRIVATE SECTION.
    DATA cut TYPE REF TO zcl_exercism_itab_nesting.
    METHODS setup.
...
ENDCLASS.
CLASS ltcl_exercism_itab_nesting IMPLEMENTATION.

  METHOD setup.
    cut = NEW zcl_exercism_itab_nesting( ).
  ENDMETHOD.
  ...
  METHOD test_empty_input.
    cl_abap_unit_assert=>assert_equals(
      act = cut->perform_nesting( artists = VALUE #( )
                                  albums = VALUE #( )
                                  songs = VALUE #( ) )
      exp = VALUE zcl_exercism_itab_nesting=>nested_data( ) ).
  ENDMETHOD.
  ...
  METHOD test_single_artist.
    cl_abap_unit_assert=>assert_equals(
      ...
      exp = VALUE zcl_exercism_itab_nesting=>nested_data( ( artist_id = '1' artist_name = 'Godsmack'
